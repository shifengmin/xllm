# DeepSeek V4 NPU Context Parallel 设计与实施计划
...
flowchart TD
  ...
  D --> E["[现有] local Q / SWA KV projection + input RoPE"]
  E --> F["[CP新增] gather real Q/KV; 丢弃 projected padding"]

  D --> G["[CP新增] gather real preprojection hidden"]
  G --> H{"layer compress ratio"}
  H -->|C1| I["[CP调整] compressor bypass"]
  H -->|C4| J["[CP新增] 4-hidden halo all-to-all/local copy"]
  J --> K["[CP调整] C4 compressor on 2B pseudo-sequences; drop warmup rows"]
  H -->|C128| L["[CP调整] C128 compressor on aligned real chunks"]

  K --> M["[CP新增] semantic allgatherv + canonical reorder"]
  L --> M
  M --> N["[CP调整] commit global compressed KV on every CP rank"]

  K --> O["[CP调整] local C4 index compress + Hadamard + quant"]
  O --> P["[CP新增] index KV/scale allgatherv + canonical commit"]
  P --> Q["[CP调整] QLI on real query with per-pseudo key endpoint"]

  F --> R["[CP新增] exchange only predecessor projected SWA KV"]
  R --> S["[CP新增] temporary PA_ND + global-logical SWA block tables"]
  F --> T["[CP新增] pack front/back as up to 2B contiguous query sequences"]
  T --> U["[CP新增] rebuild sparse/QLI metadata: local q cu-seqlens, per-pseudo seqused_kv, duplicated cmp block tables"]

  I --> V["[CP调整] sparse shared-KV attention on real query"]
  N --> V
  Q --> V
  S --> V
  U --> V
  ...
  Z --> AF["[CP新增] wait all layer collectives/cache commits; no overlap"]
  AF --> AA["[CP调整] next-layer local-padded x; layout unchanged"]

  K --> AB["[CP新增] C4 final-owner state sync"]
  L --> AC["[CP新增] C128 final-owner state sync"]
  O --> AD["[CP新增] indexer final-owner state sync"]
  R --> AE["[CP新增] final SWA window sync"]
  AB --> AF
  AC --> AF
  AD --> AF
  AE --> AF

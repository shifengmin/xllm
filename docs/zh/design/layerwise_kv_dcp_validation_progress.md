...
当前暂停点：`AllGather(Q) + owner SFA + Broadcast(output)` 验证版代码和静态 review 已完成，ATB/主仓本地提交分别为 `4f8377c`、`a63dfed07`，详见第 33 项。三轮 `python setup.py build --device npu` 均为 `BUILD_EXIT_CODE=0`，其中第三轮验证精确提交源码。机器上有其他进程，本轮未启动任何功能、语义、TPOT 或 `msprof` 测试。下次恢复先检查环境，再从 shape/路由及 ATB graph 聚焦测试开始，不要直接宣称功能已通过。`third_party/torch_npu_ops` 的既有用户状态不得清理或提交。

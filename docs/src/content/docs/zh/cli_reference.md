...
| `decode_dcp_size` | `int32` | `1` | 实验性的 NPU decode layer-owner context parallel 规模，在每个 attention TP group 内切分；该值必须整除 attention TP 规模，设为 `1` 表示禁用 decode DCP。 |
| `enable_decode_dcp_layerwise_kv_cache` | `bool` | `false` | 按 DCP layer owner 分片 NPU decode 持久 KV cache；non-owner layer 共享一个 scratch cache。要求 `decode_dcp_size > 1`。 |

...
class SfaDcpAttentionBackend(NpuPagedAttentionBackend):
    ...
    into graph-owned buffers. The captured kernels only see those addresses.
    """
    ...
    def prepare(
        ...
        *,
        ...
        # Worker slot_mapping stays logical/global. Localize a derived copy
        # for owned paged-KV writes; indexer still reads the original slots.
        ...
        query_start_loc_cpu = (
            torch.arange(num_reqs + 1, dtype=torch.int32)
            if graph_mode
            else _query_start_loc_cpu(metadata, num_reqs)
        )
        ...
        # Point the captured kernels at graph-owned storage, not builder slices
        # that would be rebuilt if the Python execute path ran again.

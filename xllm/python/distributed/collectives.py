...
# ([k*dcp_size, (k+1)*dcp_size) inside one TP block). dcp_size must divide
# tp_size so a DCP group never crosses a TP boundary. It is a separate
# communicator from ``cp`` and is mutually exclusive with kv_split.

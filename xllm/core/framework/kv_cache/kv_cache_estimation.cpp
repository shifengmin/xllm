...
int64_t layerwise_split_rank_block_bytes(
    const ModelArgs& model_args,
    int32_t layerwise_split_size,
    const KVCacheCapacity& kv_cache_cap) {
  CHECK_GT(layerwise_split_size, 1)
      << "layerwise split requires layerwise_split_size > 1.";
  ...
  for (int32_t split_rank = 0; split_rank < layerwise_split_size;
    ...
    const std::vector<bool> layer_cache_owned =
        build_layer_cache_owned(model_args, placement, num_layers);
    ...
    max_rank_bytes =
        std::max(max_rank_bytes, owned_bytes + scratch_bytes_per_block);
  }

  CHECK_GT(max_rank_bytes, 0) << "No layerwise split rank owns a model layer.";
  return max_rank_bytes;
}
...
int64_t layerwise_split_block_count(const ModelArgs& model_args,
  ...
  const int64_t per_block_bytes =
      layerwise_split_rank_block_bytes(
          model_args, layerwise_split_size, kv_cache_cap) +
      additional_block_bytes;
  const int64_t common_block_count = available_bytes / per_block_bytes;
...
void init_standard_counts(const ModelArgs& model_args,
  ...
  if (options.layerwise_split_size > 1) {
    CHECK(!use_rdma_indexer_scale_padding(options, *kv_cache_cap))
        << "layerwise split does not support RDMA scale padding.";

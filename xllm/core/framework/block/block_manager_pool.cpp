...
int32_t BlockManagerPool::get_cache_aware_dp_rank(Sequence* sequence) const {
  ...
  // The already-shared blocks the sequence holds are reused on whichever rank
  // owns them; for a fresh sequence this slice is empty.
  ...
  for (size_t i = 0; i < num_managers; ++i) {
    BlockManager* manager = block_managers_[i].get();
    matched[i] = manager->prefix_cache_match_length(sequence->tokens(),
                                                    existed_shared_blocks);
    free_blocks[i] = manager->num_free_blocks();
    const size_t used = manager->num_used_blocks();
    const size_t cached = manager->num_blocks_in_prefix_cache();
    total_blocks = std::max(total_blocks, manager->num_total_blocks());

...
class MappingNPU final {
 public:
  struct Options {
    ...
    // decode layer-owner CP size inside each attention TP group
  ...
  // Split every attention TP group into ordered contiguous DCP subgroups.
  void get_decode_dcp_group(ParallelInfo& parallel_info);
 ...
 private:
  ...
  ParallelInfo attn_decode_dcp_ = ParallelInfo();

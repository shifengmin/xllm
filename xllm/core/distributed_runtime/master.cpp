...
std::optional<std::string> validate_model_dcp(const Options& options,
  ...
  if (!Platform::is_npu()) {
    return "dcp_size > 1 is only supported on NPU";
  }
  if (!ModelConfig::is_python_model_impl(
          ModelConfig::get_instance().model_impl())) {
    return "dcp_size > 1 requires model_impl=python; Worker keeps global KV "
           "coordinates and Python SFA DCP localizes them";
  }
  ...
  if (tp_size % dcp_size != 0) {
    return "DCP groups are contiguous partitions of TP; tp_size (" +
           std::to_string(tp_size) + ") must be divisible by dcp_size (" +
...
Master::Master(const Options& options, EngineType type)
  ...
  if (dcp_size > 1) {
    ...
    LOG(INFO) << "Resolved DCP config: dcp_size=" << dcp_size
              << ", world_size=" << global_world_size

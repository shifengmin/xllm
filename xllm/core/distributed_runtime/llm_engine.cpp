...
void validate_decode_dcp_layerwise_kv_cache_model(bool enabled,
  ...
  CHECK(
      is_decode_dcp_layerwise_kv_cache_supported_model(model_args.model_type()))
      << "Decode DCP layerwise KV cache is only implemented for DeepSeek V3.2 "
         "and GLM DSA models, got model_type="
      << model_args.model_type();
  CHECK_GT(model_args.index_n_heads(), 0)
      << "Decode DCP layerwise KV cache requires an indexer-enabled model.";
  CHECK_GT(model_args.index_head_dim(), 0)
      << "Decode DCP layerwise KV cache requires a positive index head dim.";
  CHECK_GT(model_args.index_topk(), 0)
      << "Decode DCP layerwise KV cache requires a positive index top-k.";
}

#!/bin/bash
# 83 混布启动脚本 - 带 aclCreateTensor hook
# 用法: bash start_colocate_hook.sh [tag]
set -e
TAG=${1:-hook}
...
HOOK_SO=/export/home/shifengmin.3/workspace/glm5_2_pd/hook_aclcreate.so

mkdir -p $LOG_DIR
rm -f /tmp/aclCreateTensor_fail.log
...
for ((rank=0; rank<NNODES; rank++)); do
  ...
  nohup bash -lc "
    export LD_PRELOAD=$HOOK_SO:/usr/lib64/libtcmalloc.so.4:\${LD_PRELOAD:-}
...
echo "[INFO] hook log: /tmp/aclCreateTensor_fail.log"

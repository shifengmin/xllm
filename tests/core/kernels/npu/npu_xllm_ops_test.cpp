...
scratch = torch.empty(num_tokens * 64, dtype=torch.int32, device=device)
...
TEST_F(NpuXllmOpsTest, SfaDcpMergeOutKeepsBufferAcrossGraphReplay) {
  py::gil_scoped_acquire gil;
...
for num_tokens in (1, 8):
    ...
    with forward_context(ctx):
        ...
        assert torch.equal(
            eager_remap,
            remap_sparse_indices(slots, layout, index_topk=32),
        )
        torch.testing.assert_close(
            eager_merge,
        ...
        assert torch.equal(
            graph_remap,
            remap_sparse_indices(replay_slots, layout, index_topk=32),
        )
        torch.testing.assert_close(
            graph_merge,
            merge_dcp_outputs(replay_out, replay_lse),

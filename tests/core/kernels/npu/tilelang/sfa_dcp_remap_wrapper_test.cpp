...
TEST_F(SfaDcpRemapWrapperTest, MatchesNaiveCompact) {
  ASSERT_TRUE(
      has_sfa_dcp_remap_specialization(2048, kPhysicalBlockSize, kShardSize));
...
TEST_F(SfaDcpRemapWrapperTest, MatchesNaiveCompactDcp16) {
  constexpr int64_t kShardSize16 = 16;
  constexpr int64_t kShardRank16 = 7;
  ASSERT_TRUE(
      has_sfa_dcp_remap_specialization(2048, kPhysicalBlockSize, kShardSize16));
  const auto npu = torch::Device("npu:0");
  ...
  auto slots =
      torch::randint(0,
                     8 * kPhysicalBlockSize * kShardSize16,
                     {num_tokens, topk},
                     torch::TensorOptions().dtype(torch::kInt32).device(npu));
  auto mask = torch::rand({num_tokens, topk}, npu) < 0.25;
  slots = torch::where(mask, torch::full_like(slots, kInvalidSlot), slots);
  ...
  auto scratch =
      torch::empty({num_tokens * topk},
                   torch::TensorOptions().dtype(torch::kInt32).device(npu));
  sfa_dcp_remap_out(
      slots, kPhysicalBlockSize, kShardSize16, kShardRank16, out, scratch);
  auto ref =
      remap_reference(slots, kPhysicalBlockSize, kShardSize16, kShardRank16);
  EXPECT_TRUE(torch::equal(out, ref));
}

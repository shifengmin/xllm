...
def _builder() -> AscendSFADCPMetadataBuilder:
    layout = KVShardLayout.from_dcp(physical_block_size=4, dcp_size=2, dcp_rank=0)
    group = SimpleNamespace(world_size=2, rank_in_group=0, device_group=None)
...
def test_builder_graph_decode_skips_prefill_count() -> None:
    ...
    # Empty start-loc would IndexError inside _count_prefills if decode graph
    # still counted prefills on the capture path.
